...
function AddBalanceModal({ clickEvent, name, phone, availableBalance }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setIsLoading(false)
      clickEvent()
      ShowToaster(TOAST_TYPE.SUCCESS, 'Balance added successfully');
    } catch (error) {
      console.log(error);
      setIsLoading(false)
    ...
    <>
      <div className="fixed left-0 top-0 z-50 flex h-full w-full items-center justify-center bg-[rgba(0,0,0,0.5)]">
        <div className="flex font-poppins flex-col rounded-md border-0 w-[550px] bg-secondary px-12 py-5">
          ...
          <h1>Enter Amount to be added</h1>
          <div className="flex flex-col gap-5 mt-5">
            ...
            <div className="flex gap-3 ">
              <button
                ...
                disabled={isLoading}
                className=" bg-[#065F46] text-white px-3 py-1  flex gap-2 rounded text-base font-medium font-poppins"
              >
